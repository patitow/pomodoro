...
var main = function (_, Kotlin) {
  ...
  function contador(t) {
    ...
    var divcontador = Kotlin.isType(tmp$ = document.querySelector('div.contador'), HTMLDivElement) ? tmp$ : throwCCE();
    var divlongs = Kotlin.isType(tmp$_0 = document.getElementById('longbreaks'), HTMLDivElement) ? tmp$_0 : throwCCE();
    var divshorts = Kotlin.isType(tmp$_1 = document.getElementById('shortbreaks'), HTMLDivElement) ? tmp$_1 : throwCCE();
    var divciclo = Kotlin.isType(tmp$_2 = document.getElementById('countciclo'), HTMLDivElement) ? tmp$_2 : throwCCE();
    println(controle);
    if (t < 0) {
      if (longs === 1) {
      ...
      if (((t - t % 60 | 0) / 60 | 0) < 10) {
        ...
        var texto = Kotlin.isType(tmp$_3 = document.createElement('p'), HTMLParagraphElement) ? tmp$_3 : throwCCE();
        texto.textContent = min;
        divcontador.append(texto);
      } else {
        var min_0 = ((t - t % 60 | 0) / 60 | 0).toString();
        var texto_0 = Kotlin.isType(tmp$_4 = document.createElement('p'), HTMLParagraphElement) ? tmp$_4 : throwCCE();
        texto_0.textContent = min_0;
        divcontador.append(texto_0);
      }
      if (t % 60 < 10) {
        var sec = '0' + t % 60;
        var texto_1 = Kotlin.isType(tmp$_5 = document.createElement('p'), HTMLParagraphElement) ? tmp$_5 : throwCCE();
        texto_1.textContent = ':' + sec;
        divcontador.append(texto_1);
      } else {
        var sec_0 = (t % 60).toString();
        var texto_2 = Kotlin.isType(tmp$_6 = document.createElement('p'), HTMLParagraphElement) ? tmp$_6 : throwCCE();
